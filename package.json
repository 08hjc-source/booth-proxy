...
app.post("/upload", upload.single("photo"), async (req, res) => {
  try {
    ...
    if (!file) return res.status(400).json({ ok: false, message: "파일 없음" });
    ...
    if (!resp.ok) {
      ...
      return res.status(500).json({ ok: false, message: "Dropbox 업로드 실패", detail: errText });
    }

    const result = await resp.json();
    return res.json({ ok: true, path: result.path_display });
  } catch (e) {
    console.error(e);
